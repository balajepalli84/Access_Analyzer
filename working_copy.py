...
def format_audit_event(region, event):
...
headers = ["Region", "Compartment ID", "Compartment Name", "Event Name", "Auth Type", "Principal ID", "Principal Name", "Event Type"]
...
for r in regions:
    ...
    if audit_events:
        for event in audit_events:
            formatted_event = format_audit_event(r, event)
            

            for col, header in enumerate(headers):
                ws.write(row_num, col, formatted_event[header])
            row_num += 1
            # Save after writing each event to avoid data loss
